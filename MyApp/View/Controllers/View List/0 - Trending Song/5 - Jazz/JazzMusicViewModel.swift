...
class JazzMusicViewModel {
    ...
    func loadData(completion: @escaping Completion) {
        let prameters = Api.Trending.Attribute(country: "VN",
                                               limit: 30,
                                               offset: 5)
        Api.Trending.Attribute.feeDataTrendingSongJazz(parameters: prameters) { [weak self] (result) in
    ...
    func cellForRowAtTrendingSongList(at indexPath: IndexPath) -> JazzMusicTableViewCellViewModel {
        return JazzMusicTableViewCellViewModel(item: items[indexPath.row])
    }
